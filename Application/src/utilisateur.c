					...
					fd = accept(mes_services[i].socket, (struct sockaddr *) &client_addr, &addrlen);
		           	...
		           	switch(mes_services[i].type) {
						...
						case 3 :
							...
							if (pid == 0) {
								// Dialogue avec le client
								fprintf(stderr, "********** Ouverture de la session ADMINISTRATION_HOST avec [%s:%d]\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
					            administration(fd, client_addr);
					            close(fd);
					            fprintf(stderr, "********** Fermeture de la session ADMINISTRATION_HOST avec [%s:%d]\n", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));
					            exit(0);
					        }
					    break;
						close(fd);
